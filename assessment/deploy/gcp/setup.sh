...
# ffmpeg 는 **없으면 말하기 채점이 안 되는 필수품이다**(2026-08-30 시연 장애로 확인).
# 응시자 음성은 브라우저 녹음(webm)으로 들어오는데 받아쓰기·발음 평가는 wav 만 읽어서,
# 채점 서버가 입구에서 ffmpeg 로 wav 로 바꾼다. 없으면 말하기 답안이 전부 503 이 된다.
# 제대로 깔렸는지는 채점 서버 /health 의 ffmpeg_available 로 확인한다.
